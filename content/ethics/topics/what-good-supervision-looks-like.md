...
whatThisLooksLike:
  ...
  - Feedback is specific and comes soon after, not in an annual review.
  - At least one contact a month includes being observed with a client, in person where possible.
  - A supervisor checks later whether the change they asked for actually happened.
commonPitfalls:
  - Supervision that is entirely paperwork review, with nobody watching anybody work.
  - Feedback that is only praise, which tells you nothing to repeat or change.
  - A supervisor with more supervisees than they can observe, so nobody gets seen.
  - Treating direct observation as an inspection rather than the part that actually develops skill.
ifYouAreUnsure: >-
  Ask for it directly — a request to be watched running a program and told what to fix is
  always reasonable. A supervisor who cannot accommodate that over several months is a
